...
export default function ForgePanel() {
  ...
  const handleForge = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(typeof data?.error === 'string' ? data.error : 'Failed to forge entry');
      }
      setResult(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'An error occurred in the Forge.');
    } finally {
      setIsForging(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '1rem' }}>
      <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
        <h1 style={{ fontFamily: 'var(--font-heading)', color: 'var(--rarity-legendary)', fontSize: '2.5rem', margin: '0 0 0.5rem 0', textShadow: '0 0 20px rgba(245, 158, 11, 0.3)' }}>
          🔥 The Forge
        </h1>
        <p style={{ color: 'var(--color-mist)', fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto', lineHeight: 1.5 }}>
          Describe anything you accomplished. The Forge turns it into your legend — and every entry counts toward
          quests and the weekly Champion&apos;s Trophy.
        </p>
      </div>
      ...
      {!result ? (
        <div style={{ background: 'rgba(13, 13, 26, 0.6)', borderRadius: '1rem', border: '1px solid rgba(255, 255, 255, 0.05)', padding: '2rem' }}>
          ...
          {error && (
            <p style={{ color: 'var(--color-danger)', marginTop: '0.75rem', fontSize: '0.9rem' }}>{error}</p>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
            ...
            {result.cardName && (
              <div style={{ background: 'rgba(168, 85, 247, 0.1)', border: '1px solid rgba(168, 85, 247, 0.3)', color: '#a855f7', padding: '0.5rem 1rem', borderRadius: '0.5rem', fontFamily: 'var(--font-heading)', fontWeight: 'bold' }}>
                Card Gained: {result.cardName}
              </div>
            )}
            {result.leveledUp && result.newLevel !== undefined && (
              <div style={{ background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.4)', color: '#f59e0b', padding: '0.5rem 1rem', borderRadius: '0.5rem', fontFamily: 'var(--font-heading)', fontWeight: 'bold' }}>
                ⬆ Level {result.newLevel}!
              </div>
            )}
          </div>

          <p style={{ textAlign: 'center', color: 'var(--color-success)', fontSize: '0.9rem', marginBottom: '2rem' }}>
            ✓ Saved to your chronicle — this entry now counts toward quests and the weekly trophy.
          </p>
